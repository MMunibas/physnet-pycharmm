...
for ir, idcd in enumerate(iruns):
    ...
    if not os.path.exists(rfile_i) and request_reading:
        ...
        for ii, frame in enumerate(dcd.trajectory):
            ...
            for jj, (aa, bb) in enumerate(sys_rdfs):
                
                pos_trgt = frame._pos[aa]
                pos_solv = frame._pos[bb]
                dsts_atm = distance_array(
                    pos_trgt, pos_solv, box=cell).reshape(-1)
                nhst, _ = np.histogram(
...
if not os.path.exists(rfile) or request_reappending:
    ...
    for ir, idcd in enumerate(iruns):
        gfile_i = os.path.join(
            rsltdir, 'rdfs_{:s}_{:d}.npy'.format(tag, idcd))
        if os.path.exists(gfile_i):
            tmplst.append(np.load(gfile_i))
    if len(tmplst):
        nmin = np.min([len(lsti[0, :]) for lsti in tmplst])
        tmplst = np.array([
            lsti[:, :nmin] for lsti in tmplst])
        np.save(gfile, np.concatenate(tmplst))
...
# Load results
glst_mix = np.load(gfile)
Nlst = len(sys_rdfs)
glst = np.zeros((Nlst, len(glst_mix[0])), dtype=float)
Ndiv = len(glst_mix)//Nlst
for ii, glsti in enumerate(glst_mix):
    glst[ii%3, :] += glsti/Ndiv
    
# Distance binning
dst_rnge = [0.0, glst.shape[1]*dst_step]
dst_bins = np.arange(0.0, dst_rnge[-1] + dst_step/2., dst_step)
dst_cntr = dst_bins[:-1] + dst_step/2.
...
for jj, (aa, bb) in enumerate(sys_rdfs):
    ...
    axs1.plot(
        dst_cntr, glst[jj, :], ls=ls[jj], lw=3.0, 
        label=sys_rdfs_labels[jj])
    
#axs1.set_title("Radial distribution function for '{:s}'".format(tag))
axs1.set_xlabel(r'Distance $r$ ($\mathrm{\AA}$)', fontweight='bold')
axs1.get_xaxis().set_label_coords(0.5, -0.1)
axs1.set_ylabel(r'$g(r)$', fontweight='bold')
axs1.get_yaxis().set_label_coords(-0.12, 0.50)

axs1.set_xlim(0.0, 10.0)
axs1.set_ylim(-0.1, 1.9)
axs1.set_yticks(np.arange(0.0, 2.0, 0.5))
...
plt.savefig(
    os.path.join(
        rsltdir, 'rdfs_{:s}.png'.format(tag)),
    format='png', dpi=dpi)
plt.close()
